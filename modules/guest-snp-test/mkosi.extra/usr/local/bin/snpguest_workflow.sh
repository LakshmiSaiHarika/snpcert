...
snpguest_regular_attestation_workflow() {
	...
	${SNPGUEST_EXE} report ${ATTESTATION_DIR}/attestation-report.bin ${ATTESTATION_DIR}/random-request-data.txt --random

	# Fetch ARK, ASK, VCEK certificates (saved in ./certificates)
	${SNPGUEST_EXE} fetch ca pem -r ${ATTESTATION_DIR}/attestation-report.bin ${ATTESTATION_DIR}/certificates
	${SNPGUEST_EXE} fetch vcek pem ${ATTESTATION_DIR}/certificates/ ${ATTESTATION_DIR}/attestation-report.bin

	# Verify if ARK, ASK and VCEK are all signed properly
	${SNPGUEST_EXE} verify certs ${ATTESTATION_DIR}/certificates/
	${SNPGUEST_EXE} verify attestation ${ATTESTATION_DIR}/certificates/ ${ATTESTATION_DIR}/attestation-report.bin

	# Display the SNP Attestation Report(Version: 3) with random data
	${SNPGUEST_EXE} display report ${ATTESTATION_DIR}/attestation-report.bin
}
...
validate_request_data(){
	...
	local snpguest_report_request_data=$(${SNPGUEST_EXE} display report ${ATTESTATION_DIR}/attestation-report.bin \
...
validate_snp_measurement(){
	...
	local expected_measurement=$(${SNPGUEST_EXE} display report ${ATTESTATION_DIR}/attestation-report.bin \
	...
	local snpguest_report_measurement=$(${SNPGUEST_EXE} display report ${ATTESTATION_DIR}/attestation-report.bin \
